...
public extension Date {
    ...
    func localizedMonth() -> String {
        ...
        let month = formatter.string(from: self).localized(using: "Localizable")
    ...
    func localizedDate() -> String {
        ...
        if self.compare(.isToday) {
            // Label for today
            return "today".localized(using: "Localizable")
            
        } else if self.compare(.isYesterday) {
            // Label for yesterday
            return "yesterday".localized(using: "Localizable")
            
        } else {
            // Label for normal day (including day, month, year,...)
            let formatter: DateFormatter = {
                let dateFormatter = DateFormatter()
            ...
            let dayInWeek = formatter.string(from: self).localized(using: "Localizable")
            
            formatter.dateFormat = "dd"
            let dateInMonth = formatter.string(from: self)
            ...
            case "en":
                return "\(dayInWeek), \(month)/\(dateInMonth)/\(year)"
            case "vi":
                return "\(dayInWeek), \(dateInMonth)/\(month)/\(year)"
            default:
                return "\(dayInWeek), \(month)/\(dateInMonth)/\(year)"
            }
    ...
    func dateFor(_ type:DateForType) -> Date {
        ...
        case .startOfWeek:
            ...
            return adjust(.day, offset: -(offset)).adjust(hour: 0, minute: 0, second: 0)
        case .endOfWeek:
            let offset = 7 - component(.weekday)!
            return adjust(.day, offset: offset).adjust(hour: 23, minute: 59, second: 59)
        case .startOfLastWeek:
            let offset = component(.weekday)!-1
            return adjust(.day, offset: -(offset + 7)).adjust(hour: 0, minute: 0, second: 0)
        case .endOfLastWeek:
            let offset = 7 - component(.weekday)!
            return adjust(.day, offset: offset - 7).adjust(hour: 23, minute: 59, second: 59)
        case .startOfMonth:
            return adjust(hour: 0, minute: 0, second: 0, day: 1)
        case .endOfMonth:
            let month = (component(.month) ?? 0) + 1
            return adjust(hour: 0, minute: 0, second: -1, day: 0, month: month)
        case .startOfYear:
            return adjust(hour: 0, minute: 0, second: 0, day: 0, month: 1)
        case .endOfYear:
            return adjust(hour: 23, minute: 59, second: 59, day: 31, month: 12)
        case .tomorrow:
...
public enum DateForType {
    case startOfDay, endOfDay, startOfWeek, endOfWeek, startOfLastWeek, endOfLastWeek, startOfMonth, endOfMonth, startOfYear, endOfYear, tomorrow, yesterday, nearestMinute(minute:Int), nearestHour(hour:Int)
}
