...
protocol GenderPickerMenuDelegate: class {
    func genderPickerDidSelect(gender: String, genderIndex: Int)
}

class GenderPickerMenu: NSObject {
    weak var delegate: GenderPickerMenuDelegate?
    ...
    var currentGender = 0{
        didSet {
            picker.selectRow(currentGender, inComponent: 0, animated: false)
        }
    }
    ...
    func doneButtonPressed(){
        UIView.animate(withDuration: 0.5, delay: 0, usingSpringWithDamping: 1, initialSpringVelocity: 1, options: .curveEaseOut, animations: {
            ...
            self.delegate?.genderPickerDidSelect(gender: Model.UserProfileOption.genderArray[self.currentGender].localized(using: "Localizable"), genderIndex: self.currentGender)
        }
    }
...
extension GenderPickerMenu: UIPickerViewDataSource, UIPickerViewDelegate{
    func numberOfComponents(in pickerView: UIPickerView) -> Int {
        return 1
    }
    
    func pickerView(_ pickerView: UIPickerView, numberOfRowsInComponent component: Int) -> Int {
        return Model.UserProfileOption.genderArray.count
    }
    
    func pickerView(_ pickerView: UIPickerView, titleForRow row: Int, forComponent component: Int) -> String? {
        return Model.UserProfileOption.genderArray[row].localized(using: "Localizable")
    }
